...
Many underlying MVPA algorithms rely on underlying kernel computations.
However, many of the algorithms have their own implementations which are not
interchangeable, meaning that new advances in kernel logic or implementations
are not generally beneficial to PyMVPA as a whole.  To solve this, we have 
implemented a new class hierarchy which not only specifies a shared kernel
namespace, but also allows the automatic translation of kernels from one
software implementation to another.
...
automatically convert these back and forth as Shogun kernels.  This has the
