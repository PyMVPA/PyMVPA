...
\item $\K(\X,\X')$ : the matrix extension of $k$.\, i.e., $\K_{pq} =
  k(\x_p,\x'_q)$. If $\X$ is a $N \times D$ matrix and $\X'$ is $N'
  \times D$ then $\K(\X,\X1)$ is a $N \times N'$ matrix.
...
\item $\dm(\X,\X')$ : the \emph{Euclidean distance matrix} between
  ...
  $\dm(\X,\X1)$ is $N \times N'$. Note that $\dm(\X,X')$ is the square
  root of what it is usually called ``distance matrix''.
\item $\dm(\X,\X',\mathbf{w})$ : the \emph{weighted} Euclidean
  ...
  \mathbf{w}\|_2 = \sqrt{\sum_{i=1}^D w_i^2(\X_{p i} - \X'_{q i})^2}$
  through the weight vector $\mathbf{w} \in \R^D$.
...
$$\nabla_A \K = \frac{\partial \K}{\partial A} = \J_{n,m}$$
