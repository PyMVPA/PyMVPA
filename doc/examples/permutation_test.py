...
(aka *H0*) hypothesis testing for any :class:`~mvpa.measures.base.Measure`.
...
distribution.  A generic way to achieve this are permutation tests (aka *Monte
Carlo*, :ref:`Nichols et al. (2006) <NH02>`). The *Null* distribution is
estimated by computing a measure multiple times using datasets with no relevant
signal in them. For a Monte-Carlo test of a classification analyses, these
datasets are usually generated by permuting the labels of all samples in the
original training dataset each time the measure is computed, and therefore
randomize/remove any possible relevant information.

Given the results computed on permuted datasets one can now determine the
probability of the empirical measure result (i.e. the one computed from the original
...
original dataset, but has an additional attribute that indicates whether a
particular samples belongs to the part of the data that is used for training the
...
"""
Now we need the pieces for the Monte Carlo shuffling. That is first of all a
:class:`~mvpa.generators.permutation.AttributePermutator` that will permute
the target attribute of the dataset for each iteration.  Finally we instruct
to perform 200 permutations. In a real analysis the number of permutations
should be larger to get stable estimates.
"""

permutator = AttributePermutator('targets', count=200)

"""
The second mandatory piece for a Monte-Carlo-style estimation of the *Null*
distribution is the actual "estimator". :class:`~mvpa.clfs.stats.MCNullDist`
will use the :class:`~mvpa.generators.permutation.AttributePermutator` to
perform the shuffling and we tell it to later on report p-value from the left
tail of the *Null* distribution, because we are going to compute errors and are
interested in them being *lower* than chance. Finally we also ask for all result
from Monte-Carlo shuffling to be stored for subsequent visiualization of the
distribution.
"""
...
"""
Now we have a standard cross-validation result that used as usual. it has error,
...
"""
However, in addition is also provides us with the results of the statistical
evaulation. The :term:`conditional attribute` ``null_prob`` has a dataset
that contains the p-values representing the likelihood of an error equal or
lower than this one under the *Null* hypothesis, i.e. no actual signal in the
data. For a reason that will appear sensible later on, the p-value is contained
in a dataset.
"""
...
expected chance-level and the our empirical error value is in the far left tail,
hence the low p-value.

This could be the end, but sometimes one needs to look closer. Let's say you
data isn't that homogeneous. Let's say that some :term:`chunks` may be very
...
It only need a tiny change in the cross-validation setup -- the removal of the
...
"""
As you can see, we now get two errors -- one for each cross-validation fold and
...
think about what was done in the Monte Carlo analyses above.
...
each Monte Carlo iteration the classifier was tested on a new signal/data.
However, we mau be more interested in what the classifier has to say on the
...
difference is that we are now going to use a dedicate measure to estimate the
...
labeled as being par of the training set in a particular CV-fold (the
...
perform as cross-validation analysis under the *H0* hypotheses. To this end
we set of a standard CV procedure with a twist: we use a chained generator
