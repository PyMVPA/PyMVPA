...
explanation of the algoritm, and a matlab implementation, 
...
datadir='%s/pyref/' % utils._get_fingerdata_dir()
"""
...
"""
epi_ref_fn=datadir+"../glm/rall_vol00.nii"
...
pial_surf_fn=datadir+"ico%d_%sh.pial_al.asc" % (highres_ld,hemi)
white_surf_fn=datadir+"ico%d_%sh.smoothwm_al.asc" % (highres_ld,hemi)
"""
...
intermediate_surf_fn=datadir+"ico%d_%sh.intermediate_al.asc" % (lowres_ld,hemi)
"""
...
radius in milimetersa, with a variable number of voxels).
...
"""
voxel_selection_fn_prefix=datadir+"voxsel_ico%d_%sh" % (lowres_ld,hemi)
searchlight_fn_prefix=datadir+"searchlight_ico%d_%sh" % (lowres_ld,hemi)
"""
Load the surfaces
"""
...
src_ids=range(node_count) # use all nodes as a center
"""
...
distance (i.e. radius in milimeters) of each searchlight).
""" 
