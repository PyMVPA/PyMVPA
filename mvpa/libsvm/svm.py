		...
		if len(kw)==1 and kw.items()[0][0]=="svmc_parameter" and \
			...
			for attr,val in self.default_parameters.items():
				...
				if attr == 'weight_label':
					# yoh -- causes memleak. that is why I had to -DSWIG_PYTHON_SILENT_MEMLEAK
					# as a quick solution. So - XXX
					copy_val = _int_array_to_list(copy_val, svmc.svm_parameter_nr_weight_get(rval))
				elif attr == 'weight':
					copy_val = _double_array_to_list(copy_val, svmc.svm_parameter_nr_weight_get(rval))
				print "Setting %s to be %s whenever default is %s" % (attr, copy_val, val )
...
class svm_model:
	...
	def __repr__(self):
		...
		ret += ' number of classes = %d (%s),' % ( self.nr_class, `self.labels` )
		return ret+'>'
	...
	def get_param(self):
		return svm_parameter(svmc_parameter=svmc.svm_model_param_get(self.model))
