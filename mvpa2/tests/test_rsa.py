...
def test_DissimilarityMatrixMeasure():
    ...
    euc = pdist(ds.samples, 'euclidean').reshape((-1,1))
    pear = pdist(ds.samples, 'correlation').reshape((-1,1))
    city = pdist(ds.samples, 'cityblock').reshape((-1,1))
    center = pdist(ds.samples-np.mean(ds.samples,0),'correlation').reshape((-1,1))
    ...
    assert_array_almost_equal(dsm4(ds).samples,center)
